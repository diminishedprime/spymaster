...
import { ASSASSIN } from "../../../../src/constants";
import {
  cardTeamByCardId,
  cardsPath,
  currentTeamPath,
...
const userDisconnected = function*() {
  yield takeEvery<any>(t.ServerAsyncActionType.USER_DISCONNECTED, function*({
    userId
  }: t.USER_DISCONNECTEDAction) {
    yield put(afRemoveUser(userId));
    const gameIds = yield select(
      R.compose(
        R.keys,
        R.view(gamesPath)
      )
...
const userConnected = function*() {
  yield takeEvery<any>(t.ServerAsyncActionType.USER_CONNECTED, function*({
    userId
...
const flipCard = function*() {
  yield takeEvery<any>(t.ActionType.FLIP_CARD, function*({
    cardId
  }: t.FlipCard) {
...
const joinGame = function*() {
  yield takeEvery<any>(t.ActionType.JOIN_GAME, function*({
    userId,
    gameId
  }: t.JoinGame) {
    ...
    yield put(
      afBroadcastActionToUserId(userId, afSetPage(t.Page.GAME_MODE_PICK_TEAM))
    );
    yield pushGameState(gameId);
  });
};
...
const changeBackgroundColor = function*() {
  yield takeEvery<any>(t.ActionType.CHANGE_BACKGROUND_COLOR, function*({
    gameId,
    team,
    backgroundColor
