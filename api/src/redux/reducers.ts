...
const removeUser = (state: t.ServerReduxState, {userId}: t.REMOVE_USERAction) => {
...
const otherTeam = (team: t.Team) => team === t.Team.TEAM_1 ? t.Team.TEAM_2 : t.Team.TEAM_1
...
const setCardFlipped = (state: t.ServerReduxState, {cardId}: t.SetCardFlipped) =>
...
export const updateHintNumber = (state: t.ServerReduxState, {hintNumber}: t.UpdateHintNumber) =>
  R.set(hintNumberPath, hintNumber, state)

const setNewGame2Server = (state: t.ServerReduxState, {gameId, gameState}: t.NEW_GAME_SERVERAction) =>
...
const setBackgroundColor = (gameState: any, {team, backgroundColor}: t.ChangeBackgroundColor) => {
  const foregroundColor = fgForHex(backgroundColor)
  const wFg = R.set(foregroundColorForTeamPath(team), foregroundColor, gameState);
...
const gameApp = (action: any) => (gameState: any) => {
  switch (action.type) {
    case t.ServerActionType.JOIN_GAME_SERVER: return joinGameServer(gameState, action)
    case t.ServerActionType.CHANGE_BACKGROUND_COLOR_SERVER: return setBackgroundColor(gameState, action)
    case t.ServerActionType.REMOVE_USER_FROM_GAME: return removeUserFromGame(gameState, action)
    default: return gameState
  }
}

export const app = (state: t.ServerReduxState = initialState, action: t.ServerAction | t.Action) => {
  switch(action.type) {
    case t.ServerActionType.REMOVE_USER_FROM_GAME: ;
    case t.ServerActionType.CHANGE_BACKGROUND_COLOR_SERVER:
    case t.ServerActionType.JOIN_GAME_SERVER: return R.over(gameByGameId(action.action.gameId), gameApp(action), state)

    case t.ServerActionType.NEW_GAME_SERVER: return setNewGame2Server(state, action.action)
    case t.ActionType.SET_CARD_FLIPPED: return setCardFlipped(state, action.action)
    case t.ActionType.ADD_USER: return addUser(state, action.action)
    case t.ActionType.REMOVE_USER: return removeUser(state, action.action)
    case t.ActionType.UPDATE_HINT: return updateHint(state, action.action)
    case t.ActionType.SET_TIME: return setTime(state, action.action)
    case t.ActionType.UPDATE_HINT_NUMBER: return updateHintNumber(state, action.action)
    case t.ActionType.SUBMIT_HINT: return submitHint(state)
    case t.ActionType.NEXT_TURN: return nextTurn(state)
    case t.ActionType.FORFEIT: return loseGame(state, action.action)
    default:
      if (!(
